# coding:utf-8
# 设损失函数 loss = (w+1)^2, 令w初值是常数10。反向传播就是最优w, 即求最小loss对应的w的值。
...
with tf.Session() as sess:
	...
	for i in range(40):
		sess.run(train_step)
		learning_rade_val = sess.run(learning_rate)
		global_step_val = sess.run(global_step)
		w_val = sess.run(w)
		loss_val = sess.run(loss)
		print "After %s steps: global_step is %f,  w is %f , learning_rate is %f,  loss is %f." %(i, global_step_val ,w_val, learning_rade_val,loss_val)
